...
# Sikrer at tilstandsfilen lagres i samme mappe som selve skriptet (viktig for cron-jobber)
...
def get_kpi_metrics():
    ...
    for table_id in tables_jae:
        try:
            ...
            jae_code = find_code(jae_grp_var, ["kpi-jae", "uten energivarer", "jae"])
            if not jae_code:
                continue
...
def main():
    ...
    try:
        ...
        # Sjekk at vi faktisk fikk tak i data og ikke bare feilet stille
        if not latest_tid:
    ...
    # 3. Hent lenke fra RSS-feed for nyeste offisielle rapport
    ...
    # 4. Formater teksten til Slack (Korrigert til 12-måneders vekst)
    slack_text = (
